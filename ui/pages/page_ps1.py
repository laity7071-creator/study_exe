...
class PS1Page(QWidget):
    ...
    def create_cmd_card(self):
        ...
        # 子标题 + 模板
        header = QHBoxLayout()
        sub_title = QLabel("PowerShell 命令编辑")
        ...
        self.cmd_edit.setPlaceholderText("请输入PowerShell命令或脚本内容...")
        ...
        save_btn.clicked.connect(self.save_ps1_script)
    ...
    def save_ps1_script(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self,
            "保存PowerShell脚本",
            os.path.join(os.path.expanduser("~"), "ps1_script.ps1"),
            "PowerShell脚本 (*.ps1);;所有文件 (*.*)"
        )
        ...
        if file_path:
            ...
            try:
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(content)
                config_manager.increment_stat("ps1_script_count")
                show_info("成功", f"PowerShell脚本已保存到：{file_path}")
                logger.info(f"保存PS1脚本：{file_path}")
            except Exception as e:
                show_error("失败", f"保存脚本失败：{str(e)}")

    def execute_ps1(self):
        cmd = self.cmd_edit.toPlainText().strip()
        if not cmd:
            show_warn("警告", "请输入PowerShell命令！")
            return

        self.ps1_thread = PS1Thread(cmd)
        self.ps1_thread.output_received.connect(self.append_result)
        self.ps1_thread.finished.connect(self.on_ps1_finished)
        self.ps1_thread.start()

        self.exec_btn.setEnabled(False)
        self.stop_btn.setEnabled(True)
        self.result_browser.clear()
        logger.info(f"执行PS1命令：{cmd}")

    def stop_ps1(self):
        if self.ps1_thread:
            self.ps1_thread.stop()
            self.exec_btn.setEnabled(True)
            self.stop_btn.setEnabled(False)
            logger.info("停止PS1命令执行")
    ...
    def on_ps1_finished(self):
        self.exec_btn.setEnabled(True)
        self.stop_btn.setEnabled(False)
        logger.info("PS1命令执行完成")
